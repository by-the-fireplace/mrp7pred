...
outformats = {
    ...
    "sdf3000": "Symyx SDF3000",
    "sln": "Sybl line notation",
    "xyz": "XYZ",
    "iupac": "IUPAC name",
}
...
class Molecule(object):
        ...
        if not descnames:
            ...
            for descname in descnames:
                if descname not in getdescs():
                    raise ValueError(
                        "%s is not a recognised Webel descriptor type" % descname
                    )
        ans = {}
        p = re.compile(
            """Descriptor parent="(\w*)" name="([\w\-\+\d]*)" value="([\d\.]*)"""
        )
        for descname in descnames:
    ...
    def draw(self, show=True, filename=None):
        ...
        if show:
            ...
            quitbutton = tk.Button(root, text="Close", command=root.destroy).pack(
                fill=tk.X
            )
            root.mainloop()
