...
if sys.platform[:4] == "java":
    ...
    from jpype import isJVMStarted, startJVM, JException, JPackage, getDefaultJVMPath

    if not isJVMStarted():
        # work around by using PyFingerprint JVM starting mechanism
...
_aromaticityperceptor = cdk.aromaticity.Aromaticity(
    cdk.aromaticity.ElectronDonation.daylight(),
    cdk.graph.Cycles.or_(cdk.graph.Cycles.all(), cdk.graph.Cycles.all(6)),
)
...
_fingerprinters = {
    "daylight": cdk.fingerprint.Fingerprinter,
...
outformats = dict(
    [
        (_x, _formats[_x])
        for _x in list(_outformats.keys()) + ["can", "smi", "inchi", "inchikey"]
    ]
)
"""A dictionary of supported output formats"""
forcefields = list(
    cdk.modeling.builder3d.ModelBuilder3D.getInstance(_chemobjbuilder).getFfTypes()
)
...
_bondtypes = {
    1: cdk.CDKConstants.BONDORDER_SINGLE,
...
def readfile(format, filename):
    ...
    if format == "sdf":
        return (
            Molecule(mol)
            for mol in cdk.io.iterator.IteratingSDFReader(
...
class Molecule(object):
    ...
    def atoms(self):
        return [
            Atom(self.Molecule.getAtom(i)) for i in range(self.Molecule.getAtomCount())
        ]
    ...
    def calcdesc(self, descnames=[]):
        ...
        for descname in descnames:
            try:
                ...
                raise ValueError(
                    "%s is not a recognised CDK descriptor type" % descname
                )
            try:
        ...
        if not usecoords:
            ...
            if self.atoms[0].Atom.getPoint2d() is None:
                ...
                for atom, newatom in zip(self.atoms, mol.atoms):
                    ...
                    newatom.Atom.setPoint2d(javax.vecmath.Point2d(coords.x, coords.y))
...
if sys.platform[:4] == "java":
    ...
    class _Canvas(javax.swing.JPanel):
        ...
        def __init__(self, mol):
            ...
            generators.append(
                cdk.renderer.generators.standard.StandardGenerator(
                    java.awt.Font("Verdana", java.awt.Font.PLAIN, 18)
                )
            )
            self.renderer = cdk.renderer.AtomContainerRenderer(
                generators, cdk.renderer.font.AWTFontManager()
            )
            model = self.renderer.getRenderer2DModel()
            model.set(
                cdk.renderer.generators.standard.StandardGenerator.AtomColor,
                cdk.renderer.color.CDK2DAtomColors(),
            )
        ...
        def paint(self, g):
            ...
            self.renderer.paint(
                self.mol,
                cdk.renderer.visitor.AWTDrawVisitor(g),
                java.awt.Rectangle(300, 300),
                True,
            )
