...
def GetInertiaMatrix(mol, ChargeCoordinates):
    ...
    res13 = 0.0
    for i in range(nAT):
        res11 = res11 + temp[i][0] * (
            math.pow(temp[i][1][1], 2) + math.pow(temp[i][1][2], 2)
        )
        res22 = res22 + temp[i][0] * (
            math.pow(temp[i][1][0], 2) + math.pow(temp[i][1][2], 2)
        )
        res33 = res33 + temp[i][0] * (
            math.pow(temp[i][1][0], 2) + math.pow(temp[i][1][1], 2)
        )
