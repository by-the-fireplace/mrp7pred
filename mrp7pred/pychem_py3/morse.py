    ...
    *Internal Use Only*
    ...
    for i in range(NAtom - 1):
        for j in range(i + 1, NAtom):
            DistanceMatrix[i, j] = _GetAtomDistance(
                CoordinateList[i], CoordinateList[j]
            )
...
def CalculateChargeMoRSE(ChargeCoordinates):
    ...
    for kkk, Ri in enumerate(R):
        ...
        for j in range(nAT - 1):
            for k in range(j + 1, nAT):
                res = res + charge[j] * charge[k] * math.sin(Ri * DM[j, k]) / (
                    Ri * DM[j, k]
                )
...
def CalculateMassMoRSE(mol, ChargeCoordinates):
    ...
    for kkk, Ri in enumerate(R):
        ...
        for j in range(nAT - 1):
            for k in range(j + 1, nAT):
                res = res + mass[j] * mass[k] * math.sin(Ri * DM[j, k]) / (
                    Ri * DM[j, k]
                )
...
def CalculatePolarizabilityMoRSE(ChargeCoordinates):
    ...
    for kkk, Ri in enumerate(R):
        ...
        for j in range(nAT - 1):
            for k in range(j + 1, nAT):
                res = res + polarizability[j] * polarizability[k] * math.sin(
                    Ri * DM[j, k]
                ) / (Ri * DM[j, k])
