...
def GetWHIM10(CoordinateMatrix, AtomLabel, proname="u"):
    ...
    res = (
        scipy.power(s[0], 2) * nAtom / sum(scipy.power(S * scipy.matrix(u[:, 0]).T, 4))
    )
...
def GetWHIM11(CoordinateMatrix, AtomLabel, proname="u"):
    ...
    res = (
        scipy.power(s[1], 2) * nAtom / sum(scipy.power(S * scipy.matrix(u[:, 1]).T, 4))
    )
...
def GetWHIM12(CoordinateMatrix, AtomLabel, proname="u"):
    ...
    res = (
        scipy.power(s[2], 2) * nAtom / sum(scipy.power(S * scipy.matrix(u[:, 2]).T, 4))
    )
